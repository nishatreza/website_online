...
times: string[] = ['Upto 24 hours', 'Upto 3 days', 'Upto 7 days', 'Any'];
 ...
 categories: string[] = ['Graphics',
   ...
   'Programming & Tech', 'Business', 'Fun & Lifestyle'];
 ...
 url: string;
  ...
  onSubmit(form:any) {
  this.serviceCollection = this.afs.collection('serviceInfo');
  ...
  public getDocWithId(collection, key, value) {
    ...
    const documentWithId = collectionRef.snapshotChanges().pipe(
      map(actions => actions.map(a => {
        ...
        const serviceId = a.payload.doc.id;
  ...
  public updateDoc(options): any {
    this.getDocWithId('serviceInfo', options.serviceId, this.serviceId).subscribe(
      data => {
        const docId = data[0].serviceId;
        options.currentDoc = data[0];
        return this.afs.collection('serviceInfo').doc(docId).update(options.updatePayload).then(
          () => {
  ...
  firstDropDownChanged(val: any) {
    ...
    if (val == "Graphics") {
      this.categoryTypes = ["Logo Design", "Flyers & Brochures", "Packaging Design", "Book & Album Covers",
        "Business Cards & Stationary", "Web & Mobile Design", "Presentation Design", "Infographics",
        "Cartoons & Caricatures", "3D & 2D Models", "Tshirts & Merchandise", "Photoshop & Editing", "Banner Ads", "Social Media Design",
        "Vector Tracing", "Invitations"];
    }
    else if (val == "Digital Marketing") {
      this.categoryTypes = ["SEO", "Social Media Marketing", "Email Marketing", "Content Marketing",
        "Video Marketing", "Web Analytics", "Local Listings", "Domain Research", "Search & Display Marketing",
        "Marketing Strategy","Influencer Marketing", "Web Traffic", "Mobile Advertising", "Music Promotion"];
    }
    else if (val == "Writing & Translation") {
      this.categoryTypes = ["Articles & Blog Posts", "Business Copywriting", "Resumes & Cover Letters",
        "Translation", "Creative Writing", "ProofReading & Editing", "Press Releases",
        "Transciption", "Legal Writing"];
    }
    else if (val == "Video & Animation") {
      this.categoryTypes = ["Whiteboard & Animated Explainers", "Intros & Animated Logos", "Promotional Videos",
        "Live Action Explainers", "Short Video Ads", "Spokesperson Videos", "Editing & Post Production",
        "Lyrics & Music Videos", "Animated Characters & Modelling"];
    }

    else if (val == "Music & Audio") {
      this.categoryTypes = ["Voice Over", "Mixing & Mastering", "Producers & Composers", "Singer & Songwriters", "Sound Effects",
        "Session Musicians & Singers", "Jingles & Drops"]
    }
    else if (val == "Programming & Tech") {
      this.categoryTypes = ["WordPress", "Web Programming", "Ecommerce", "Mobile Apps & Web", "Website Builders & CMS",
        "Desktop Applications", "Data Analysis & Reports", "Convert Files", "Support & IT", "Chatbots", "Databases", "User Testing",
        "QA"]
    }

    else if (val == "Business") {

      this.categoryTypes = ["Virtual Assistant", "Market Research", "Business Plans", "Branding Services", "Legal Consulting",
        "Financial Consulting", "Business Tips", "Presentations", "Career Advice", "Flyer Distribution"]
    }
    else if (val == "Fun & lifestyle") {
      this.categoryTypes = ["Online Lessions", "Arts & Crafts", "Relationship Advice", "Health,Nutrition & Fitness",
        "Astrology & Readings", "Spiritual & Healing", "Family & Genealogy", "Collectibles", "Greeting Cards & Videos",
        "Your Message On...", "Viral Videos", "Pranks & Stunts", "Celebrity Impersonators", "Gaming", "Global Culture"]
    }
